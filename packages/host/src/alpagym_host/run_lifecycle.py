...
def _start_wizard_process(
    ...
    if len(host.alpasim_gpu_ids) == 1:
        ...
        wizard_command.extend(
            [
                f"services.renderer.gpus={gpu_ids}",
                f"services.driver.gpus={gpu_ids}",
                f"services.physics.gpus={gpu_ids}",
                f"services.trafficsim.gpus={gpu_ids}",
            ]
        )
...
def _build_cosmos_command(
    ...
    return build_cosmos_srun_command(
        ...
        runtime_check_command=[
            "/opt/venv/bin/python",
            "-c",
            "import alpagym_host; import alpagym_runtime; import alpagym_g1_vla",
        ],
...
def _build_cosmos_launcher_command(
    ...
    if use_baked_runtime:
        command = ["/opt/venv/bin/python", "-m"]
    else:
        command = [
            "uv",
            "run",
