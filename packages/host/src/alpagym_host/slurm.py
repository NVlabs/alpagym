...
def build_wizard_srun_command(
    ...
    srun_command = [
        ...
        f"--gpus-per-task={slurm.gpus_per_node}",
        f"--gpu-bind=mask_gpu:{_gpu_mask(tuple(range(slurm.gpus_per_node)))}",
    ]
...
def _cosmos_launcher_script(
    ...
    """
    lines = [shlex.join(runtime_check_command), 'case "$SLURM_PROCID" in']
