...
"""Policy-owned replay collation for Wenhao's ragged Qwen3-VL inputs."""
...
def collate_wenhao_replay_samples(
    ...
    *,
    ...
    """
    if not samples:
        raise ValueError("Wenhao replay collation requires at least one sample")
    if isinstance(pad_token_id, bool) or not isinstance(pad_token_id, int):
        raise TypeError("Wenhao pad_token_id must be an integer")
    if pad_token_id < 0:
        raise ValueError("Wenhao pad_token_id must be non-negative")
    ...
    if any(set(sample.model_inputs) != first_keys for sample in samples):
        raise ValueError("Wenhao replay model input keys differ across samples")
    missing = sorted(required_keys - first_keys)
    if missing:
        raise ValueError(
            "Wenhao replay model inputs are missing required fields: "
            + ", ".join(missing)
        )
    ...
    if collisions:
        raise ValueError(
            "Wenhao replay inputs contain collator-owned fields: "
            + ", ".join(collisions)
        )
    ...
    for sample_index, sample in enumerate(samples):
        ...
        if not all(
            ...
            raise TypeError("Wenhao ragged replay fields must be tensors")
        if ids.ndim != 1 or ids.numel() == 0:
            raise ValueError(
                f"Wenhao input_ids for sample {sample_index} must be non-empty [S]"
            )
        if ids.dtype not in (
            torch.uint8,
            torch.int8,
            ...
            raise TypeError("Wenhao input_ids must use an integer dtype")
        if bool((ids < 0).any()):
            raise ValueError("Wenhao input_ids must be non-negative")
        if attention.ndim != 1 or attention.shape != ids.shape:
            raise ValueError(
                f"Wenhao attention_mask for sample {sample_index} must match input_ids"
            )
        if attention.dtype not in (
            ...
            raise TypeError("Wenhao attention_mask must use a bool or integer dtype")
        ...
        if pixels.ndim != 2 or pixels.shape[0] == 0 or pixels.shape[1] == 0:
            raise ValueError(
                f"Wenhao pixel_values for sample {sample_index} must be non-empty [P,F]"
            )
        if not torch.is_floating_point(pixels):
            raise TypeError("Wenhao pixel_values must use a floating-point dtype")
        if not bool(torch.isfinite(pixels).all()):
            raise ValueError("Wenhao pixel_values contain non-finite values")
        if grid.ndim != 2 or grid.shape[0] == 0 or grid.shape[1] != 3:
            raise ValueError(
                f"Wenhao image_grid_thw for sample {sample_index} must be non-empty [I,3]"
            )
        if grid.dtype not in (
            ...
            raise TypeError("Wenhao image_grid_thw must use an integer dtype")
        ...
        if pixels.shape[0] != expected_pixel_rows:
            raise ValueError(
                f"Wenhao pixel_values rows for sample {sample_index} "
        ...
        if first_input_dtype is None:
            ...
            raise ValueError(
                "Wenhao ragged replay tensor dtype, device, or pixel width "
                "differs across samples"
            )
        if attention.device != ids.device:
            raise ValueError("Wenhao input_ids and attention_mask must share a device")
        ...
        if pooling_keys <= first_keys:
            ...
            if not isinstance(effective_grid, torch.Tensor) or not isinstance(
                ...
                raise TypeError("Wenhao visual pooling metadata must be tensors")
            if effective_grid.shape != grid.shape:
                raise ValueError(
                    "Wenhao effective_image_grid_thw must match image_grid_thw rows"
                )
            if effective_grid.dtype not in (
                ...
                raise TypeError(
                    "Wenhao effective_image_grid_thw must use an integer dtype"
                )
            if not bool((effective_grid > 0).all()):
                raise ValueError(
                    "Wenhao effective_image_grid_thw entries must be positive"
                )
            if pool_factors.ndim != 1 or pool_factors.shape[0] != grid.shape[0]:
                raise ValueError(
                    "Wenhao visual_pool_factors must have one entry per image"
                )
            if pool_factors.dtype not in (
                ...
                raise TypeError("Wenhao visual_pool_factors must use an integer dtype")
            if not bool((pool_factors > 0).all()):
                raise ValueError("Wenhao visual_pool_factors must be positive")
            if sample_index == 0:
                ...
                raise ValueError(
                    "Wenhao visual pooling metadata dtype or device differs "
                    "across samples"
                )
            if (
                ...
                raise ValueError(
                    "Wenhao image grids and visual_pool_factors must share a device"
                )
    ...
    if primitive_widths:
        if len(primitive_widths) != len(rectangular_samples):
            raise ValueError(
                "Wenhao primitive reward signals mix present and missing rows"
            )
