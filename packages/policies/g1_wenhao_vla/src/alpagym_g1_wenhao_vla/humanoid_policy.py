...
"""Wenhao VLA callback using AlpaGym's existing motion-reference path."""
...
from alpagym_g1_wenhao_vla.bundle import MODEL_FAMILY, REPLAY_SCHEMA
...
from alpagym_g1_wenhao_vla.inference_model import unwrap_actor_critic
...
_WENHAO_ACTION_ROWS = 30
_WENHAO_ACTION_WIDTH = 38
_RTC_INITIAL_DELAY_ROWS = 6
_RTC_DELAY_WINDOW_SIZE = 6
_WENHAO_CONTROL_HZ = 30
_SONIC_CONTROL_HZ = 50
...
class G1WenhaoVlaHumanoidPolicy:
    ...
    def __init__(
        ...
        *,
        ...
        self._reference_adapter = WenhaoH50ReferenceAdapter(
            humanoid_repo_path,
            planner_sha256=humanoid_planner_sha256,
        ...
        if self._reference_adapter.joint_names != MOTION_REFERENCE_JOINT_NAMES:
            raise ValueError(
                "Wenhao converter joint order differs from AlpaGym's G1 contract"
            )
        scenario_id = str(request.scenario_id)
        if not scenario_id.strip():
            raise ValueError("Wenhao policy requires a non-empty scenario_id")
    ...
    def step(
        ...
        *,
        ...
        if (
            ...
            raise ValueError(
                "Wenhao attested checkpoint and native reference adapter disagree "
                "on RTC max_delay"
            )
        if _RTC_INITIAL_DELAY_ROWS >= actor_critic.rtc_max_delay_exclusive:
            raise ValueError(
                "Wenhao initial RTC delay is outside the attested checkpoint contract"
            )
        if actor_critic.schedule.sha256 != self._expected_schedule_sha256:
            raise ValueError("Wenhao leased model flow schedule identity changed")
        if actor_critic.noise_level != WENHAO_FLOW_NOISE_LEVEL:
            raise ValueError(
                f"Wenhao v1 requires Flow-SDE noise_level={WENHAO_FLOW_NOISE_LEVEL}"
            )
        if actor_critic.ignore_last is not WENHAO_FLOW_IGNORE_LAST:
            raise ValueError("Wenhao v1 requires Flow-SDE ignore_last=true")
    ...
    def _lane(
        ...
        actor_critic: WenhaoPsiActorCritic,
    ) -> _Lane:
        ...
        if lane is not None:
            if lane.reset_episode_id != int(policy_input.episode_id):
                raise ValueError("Wenhao lane crossed reset IDs inside one session")
            return lane
        ...
        lane = _Lane(
            generator=generator,
            history=WenhaoImageHistory(episode_index=self._bats_episode_index),
    ...
    def _validate_lifecycle(
        ...
        *,
        ...
        if policy_input.step_index == 0 and policy_input.feedback_trace is not None:
            raise ValueError("initial Wenhao input unexpectedly carries feedback")
        if policy_input.step_index > 0 and policy_input.feedback_trace is None:
            raise ValueError("Wenhao replan/FINALIZE input is missing feedback")
        if sample_actions and policy_input.bootstrap_requested:
            raise ValueError(
                "Wenhao bootstrap value cannot be requested while sampling"
            )
    ...
    def _rtc_inputs(
        *,
        ...
        device = actor_critic.normalizer.state_low.device
        actions = torch.zeros(
            (1, _WENHAO_ACTION_ROWS, _WENHAO_ACTION_WIDTH),
            dtype=torch.float32,
            device=device,
        )
        mask = torch.zeros((1, _WENHAO_ACTION_ROWS), dtype=torch.bool, device=device)
        rtc_state = lane.rtc_state
        if rtc_state is None or not rtc_state.delay_rows:
            raise AssertionError("Wenhao RTC lane state is missing")
        if len(rtc_state.delay_rows) > _RTC_DELAY_WINDOW_SIZE or any(
            value < 0 for value in rtc_state.delay_rows
        ):
            raise AssertionError("Wenhao RTC delay history is invalid")
        ...
        if (
            ...
            != (
                _WENHAO_ACTION_ROWS,
                _WENHAO_ACTION_WIDTH,
            )
            or not torch.isfinite(previous_chunk).all()
        ):
            raise AssertionError("Wenhao RTC previous chunk is invalid")
        if (
            ...
            raise AssertionError("Wenhao RTC previous joint targets are invalid")
        ...
        if trace is None:
            raise ValueError("Wenhao RTC replan is missing realized feedback")
        if int(trace.env_id) != int(policy_input.env_id):
            raise ValueError("Wenhao RTC feedback targets a different env lane")
        predecessor_tick_count = _initial_predecessor_tick_count(
            trace,
            installed_reference_id=previous.reference_id,
        )
        # Feedback is on SONIC's 50 Hz clock.  Ceil rather than round so the
        # delay predictor never exposes a row that may still be executing.
        observed_delay_rows = (
            predecessor_tick_count * _WENHAO_CONTROL_HZ + _SONIC_CONTROL_HZ - 1
        ) // _SONIC_CONTROL_HZ
        next_delay_rows = (
        ...
        if source_suffix.shape[0] > 0:
            ...
            if tuple(qpos.shape) != (36,) or not torch.isfinite(qpos).all():
                raise ValueError("Wenhao RTC current qpos must be finite shape (36,)")
    ...
    def _observation_inputs(
        ...
        *,
        ...
        if len(policy_input.camera_frames) != 1:
            raise ValueError("Wenhao policy requires exactly one current D455 frame")
    ...
    def _validate_same_shot_state(
        ...
        if frame.env_id != policy_input.env_id:
            raise ValueError("Wenhao D455 frame targets a different env lane")
        ...
        if tuple(qpos.shape) != (36,) or tuple(render_qpos.shape) != (36,):
            raise ValueError("Wenhao G1 qpos and render_qpos must both be 36-D")
        if not torch.equal(qpos, render_qpos):
            raise ValueError("Wenhao image and policy qpos are not the same shot")
    ...
    def _sample_output(
        ...
        *,
        lane: _Lane,
        policy_input: HumanoidPolicyInput,
        sample: WenhaoPolicySample | Any,
        replay_visual: dict[str, torch.Tensor],
        rtc: _RtcInputs,
        actor_critic: WenhaoPsiActorCritic,
    ) -> HumanoidPolicyStepOutput:
        """Materialize the H50 SONIC buffer while retaining the raw Flow action."""
        denormalized_wire = torch.as_tensor(sample.denormalized_wire)
        if tuple(denormalized_wire.shape) != (1, 30, 38):
            raise ValueError("Wenhao sampled wire chunk must have shape [1,30,38]")
        ...
        if (
            ...
            raise ValueError(
                "Wenhao sampled clipped-normalized chunk must have shape [1,30,38]"
            )
        if torch.any((sampled_clipped < -1.0) | (sampled_clipped > 1.0)):
            raise ValueError("Wenhao sampled clipped-normalized chunk left [-1,1]")
        if bool(rtc.mask.any()) and not torch.equal(
            ...
            raise AssertionError("Wenhao Flow sampler changed the fixed RTC prefix")
        ...
        payload.update(
            {
                ...
                "humanoid": {"wenhao_raw_action_rows": rows.clone()},
            }
        )
        ...
        if trace is not None:
            ...
            if tuple(chain.shape[:2]) != (1, 11):
                raise ValueError("Wenhao sampled chain must contain 10 denoise steps")
            required = (
                "clipped_normalized",
            ...
            if missing:
                raise ValueError(
                    f"Wenhao trainable sample is missing Flow replay fields {missing}"
                )
            old_element_logprobs = torch.as_tensor(sample.old_element_logprobs)
            if tuple(old_element_logprobs.shape) != (
                1,
                _WENHAO_ACTION_ROWS * _WENHAO_ACTION_WIDTH,
            ):
                raise ValueError(
                    "Wenhao sampled element logprobs must have shape [1,1140]"
                )
            masked_elements = rtc.mask[..., None].expand(-1, -1, _WENHAO_ACTION_WIDTH)
            if not torch.equal(
                ...
                raise AssertionError(
                    "Wenhao fixed RTC prefix contributed rollout density"
                )
            payload.update(
                {
                    ...
                    "flow_noise_level": WENHAO_FLOW_NOISE_LEVEL,
                    "flow_ignore_last": WENHAO_FLOW_IGNORE_LAST,
        ...
        return HumanoidPolicyStepOutput(
            ...
            replay_data=replay,
            model_extra={
                "wenhao_history_frame_count": int(
                    replay_visual["selected_history_indices"].numel()
                ),
                "wenhao_rtc_delay_rows": int(rtc.mask.sum().item()),
                "wenhao_rtc_source_cursor_h50": rtc.source_cursor_h50,
                "wenhao_rtc_source_start_row_h30": rtc.source_start_row_h30,
                "wenhao_raw_action_rows": 30,
                "wenhao_reference_frames": H50_FRAME_COUNT,
                "wenhao_replan_controller_ticks": REPLAN_CONTROLLER_TICKS,
            },
        )

    @staticmethod
    def _autocast(actor_critic: WenhaoPsiActorCritic) -> torch.autocast:
...
def build_humanoid_policy_factory(
    ...
    """Build motion-reference Wenhao sessions from strict bundle config."""
    config = run_config.policy.model.bundle_config
    if not isinstance(config, Mapping):
        raise TypeError("Wenhao bundle_config must be a mapping")
    ...
    humanoid_planner_sha256 = _required_sha(config, "humanoid_wenhao_planner_sha256")
    ...
    if camera_format not in {"png", "jpeg"}:
        raise ValueError("Wenhao camera_image_format must be png or jpeg")
    camera_contract_sha256 = _required_sha(config, "camera_contract_sha256")

    def _factory(session_uuid: str, request: Any) -> G1WenhaoVlaHumanoidPolicy:
        ...
        return G1WenhaoVlaHumanoidPolicy(
            ...
            humanoid_planner_sha256=humanoid_planner_sha256,
...
def _validate_session_request(
    ...
    *,
    ...
    if int(request.execution_mode) != HUMANOID_EXECUTION_MODE_MOTION_REFERENCE:
        raise ValueError("Wenhao policy requires motion-reference execution mode")
    if tuple(str(name) for name in request.joint_names) != MOTION_REFERENCE_JOINT_NAMES:
        raise ValueError("Wenhao policy joint order differs from canonical IsaacLab")
    if int(request.action_size) != 0:
        raise ValueError("Wenhao motion-reference action_size must be zero")
    ...
    if actual != expected:
        raise ValueError("Wenhao H50 motion-reference contract changed")
    if str(request.action_schema) != _H50_SCHEMA:
        raise ValueError("Wenhao action_schema must match the H50 reference schema")
    ...
    if camera_actual != camera_expected:
        raise ValueError("Wenhao D455 camera contract changed")
    for field in ("attempt_id", "scene_id", "scenario_id"):
        if not str(getattr(request, field)):
            raise ValueError(f"Wenhao session requires non-empty {field}")
    ...
    *,
    ...
    for label, value in (
        ...
        if isinstance(value, bool):
            raise TypeError(f"Wenhao RTC {label} timestamp must be an integer")
        timestamp = int(value)
        if timestamp != value or timestamp < 0:
            raise ValueError(
                f"Wenhao RTC {label} timestamp must be a non-negative integer"
            )
        timestamps.append(timestamp)
    source, current = timestamps
    if current <= source:
        raise ValueError("Wenhao RTC source timestamps must be strictly increasing")
    elapsed_us = current - source
    if elapsed_us % REFERENCE_PERIOD_US != 0:
        raise ValueError("Wenhao RTC source time is not on the exact 20 ms H50 grid")
    ...
    start_row_h30 = min(
        (cursor_h50 * _WENHAO_CONTROL_HZ + _SONIC_CONTROL_HZ - 1) // _SONIC_CONTROL_HZ,
        _WENHAO_ACTION_ROWS,
    )
...
def _initial_predecessor_tick_count(
    ...
    *,
    ...
    if len(predecessor_ids) > 1:
        raise ValueError(
            "Wenhao RTC feedback switched between multiple predecessor references"
        )
    if any(
        int(tick.active_reference_id) != int(installed_reference_id)
        for tick in ticks[first_installed:]
    ):
        raise ValueError("Wenhao RTC feedback left the newly installed reference")
...
def _rebase_normalized_chunk_relative_dims(
    ...
    *,
    ...
    """Re-express Wenhao dims 29:38 in the current yaw-relative base.

    Joint targets are absolute and remain untouched.  Rotation reconstruction
    follows Wenhao's yaw-only convention; local translation continues to use
    the full base rotation, matching the authoritative deployment server.
    """
    chunk = (
        torch.as_tensor(normalized_chunk).detach().to(device="cpu", dtype=torch.float64)
    )
    if chunk.ndim != 2 or int(chunk.shape[1]) != _WENHAO_ACTION_WIDTH:
        raise ValueError("Wenhao RTC chunk must have shape [H,38]")
    if not torch.isfinite(chunk).all():
        raise ValueError("Wenhao RTC chunk contains non-finite values")
    low = (
    ...
    if tuple(low.shape) != (_WENHAO_ACTION_WIDTH,) or tuple(high.shape) != (
        _WENHAO_ACTION_WIDTH,
    ):
        raise ValueError("Wenhao RTC action bounds must have shape (38,)")
    action_range = high - low
    if not torch.all(action_range > 0):
        raise ValueError("Wenhao RTC action bounds are degenerate")
...
def _quaternion_wxyz_to_rotation(quaternion: torch.Tensor) -> torch.Tensor:
    ...
    if tuple(q.shape) != (4,) or not torch.isfinite(q).all():
        raise ValueError("Wenhao RTC base quaternion must be finite shape (4,)")
...
def _rotation_6d_to_matrix(rotation_6d: torch.Tensor) -> torch.Tensor:
    """Project two rotation columns onto SO(3), matching Wenhao deployment."""
    value = torch.as_tensor(rotation_6d, dtype=torch.float64).reshape(-1)
    if tuple(value.shape) != (6,) or not torch.isfinite(value).all():
        raise ValueError("Wenhao RTC rotation-6D value must be finite shape (6,)")
    first = value[:3]
...
def _required_text(config: Mapping[str, Any], name: str) -> str:
    ...
    if not value.strip():
        raise ValueError(f"Wenhao bundle_config.{name} must be non-empty")
    return value
...
def _required_sha(config: Mapping[str, Any], name: str) -> str:
    ...
    if _SHA256.fullmatch(value) is None:
        raise ValueError(f"Wenhao bundle_config.{name} must be lowercase SHA-256")
    return value
