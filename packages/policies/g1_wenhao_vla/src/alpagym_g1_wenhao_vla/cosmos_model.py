...
"""Cosmos-RL lifecycle wrapper for the attested Wenhao Psi actor-critic."""
...
from alpagym_g1_wenhao_vla.provenance import (
    ARGV_SHA256,
...
WENHAO_PSI_PPO_MODEL_TYPE = "g1_wenhao_psi_ppo"
...
class WenhaoPsiPPOConfig(PretrainedConfig):
    """HF-shaped, content-addressed configuration for Wenhao Psi PPO."""

    model_type = WENHAO_PSI_PPO_MODEL_TYPE
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        for label, (actual, expected) in expected_pins.items():
            if actual != expected:
                raise ValueError(
                    f"Wenhao {label} does not match the pinned bundle: "
        ...
        for label in (
            ...
            if _SHA256_PATTERN.fullmatch(str(locals()[label])) is None:
                raise ValueError(f"Wenhao {label} must be a lowercase SHA-256")
        if normalization_type != "bounds_q99":
            raise ValueError("Wenhao v1 requires normalization_type='bounds_q99'")
        if float(flow_noise_level) != WENHAO_FLOW_NOISE_LEVEL:
            raise ValueError(
                f"Wenhao v1 requires flow_noise_level={WENHAO_FLOW_NOISE_LEVEL}"
            )
        if flow_ignore_last is not WENHAO_FLOW_IGNORE_LAST:
            raise ValueError("Wenhao v1 requires flow_ignore_last=true")
        if (
            ...
            raise ValueError(
                "Wenhao pinned checkpoint requires exclusive RTC max_delay="
                f"{_NATIVE_RTC_MAX_DELAY_EXCLUSIVE}"
            )
        if vlm_hidden_dim <= 0:
            raise ValueError("Wenhao vlm_hidden_dim must be positive")
        hidden_sizes = [int(width) for width in critic_hidden_sizes]
        if not hidden_sizes or min(hidden_sizes) <= 0:
            raise ValueError("Wenhao critic_hidden_sizes must be positive")
        ...
        if schedule.sha256 != flow_schedule_sha256:
            raise ValueError(
                "Wenhao flow_schedule_sha256 does not identify the configured grid"
            )
...
class WenhaoPsiPPOModel(BaseModel):
    """Single-GPU Cosmos wrapper around :class:`WenhaoPsiActorCritic`."""

    actor_critic: WenhaoPsiActorCritic

    def __init__(self, hf_config: WenhaoPsiPPOConfig) -> None:
        """Build the attested Psi architecture without allocating 2B weights."""
        super().__init__(hf_config)
        self.config = hf_config
        self.source_bundle = WenhaoSourceBundle.verify(hf_config.policy_eval_root)
        if self.source_bundle.model_root.name != hf_config.bundle_model_id:
            raise ValueError(
                "Verified Wenhao model root does not match bundle_model_id"
            )
        ...
        if any(parameter.device.type != "meta" for parameter in psi_model.parameters()):
            raise RuntimeError(
                "Wenhao Psi must be constructed inside Cosmos's meta init context"
            )
        ...
        with torch.device("cpu"), cosmos_default_dtype(torch.float32):
            ...
            normalizer = WenhaoQ99Normalizer.from_stats_file(
                self.source_bundle.stats_path
            )
        self.actor_critic = WenhaoPsiActorCritic(
            psi_model=psi_model,
            schedule=schedule,
    ...
    def supported_model_types() -> list[str]:
        ...
        return [WENHAO_PSI_PPO_MODEL_TYPE]
    ...
    def apply_pipeline_split(self, pp_rank: int, pp_size: int) -> None:
        ...
        del pp_rank
        if pp_size != 1:
            raise NotImplementedError("Wenhao Psi v1 does not support PP")
    ...
    def check_cp_compatible(self, cp_size: int, tp_size: int) -> None:
        ...
        if cp_size != 1 or tp_size != 1:
            raise NotImplementedError("Wenhao Psi v1 does not support CP or TP")

    def check_tp_compatible(self, tp_size: int) -> None:
        """Reject tensor parallel layouts larger than one."""
        if tp_size != 1:
            raise NotImplementedError("Wenhao Psi v1 does not support TP")
    ...
    def load_hf_weights(
        ...
        if any(tensor.device.type == "meta" for tensor in targets.values()):
            raise RuntimeError("Wenhao Psi weights must be materialized before loading")
        if any(tensor.device != device for tensor in targets.values()):
            raise RuntimeError("Wenhao Psi target tensors differ from the load device")
        ...
        with safe_open(
            ...
            if missing or extra:
                raise RuntimeError(
                    "Wenhao checkpoint does not strictly match Psi architecture; "
            ...
            for name, target in targets.items():
                ...
                if source_shape != tuple(target.shape):
                    raise RuntimeError(
                        f"Wenhao tensor shape mismatch for {name}: "
    ...
    def from_pretrained(
        ...
        if not isinstance(hf_config, WenhaoPsiPPOConfig):
            raise TypeError("Wenhao Cosmos model requires WenhaoPsiPPOConfig")
    ...
    def clone_for_inference_lease(self) -> WenhaoPsiActorCritic:
        """Return the core's isolated inference snapshot."""
        return self.actor_critic.clone_for_inference_lease()


def register_wenhao_psi_ppo_model() -> None:
    """Idempotently register the config, local model root, and Cosmos model."""
    global _LOCAL_CONFIG_REGISTERED
    try:
        AutoConfig.register(WENHAO_PSI_PPO_MODEL_TYPE, WenhaoPsiPPOConfig)
    except ValueError as exc:
        if "is already used" not in str(exc) and "already exists" not in str(exc):
            raise
    if CONFIG_MAPPING[WENHAO_PSI_PPO_MODEL_TYPE] is not WenhaoPsiPPOConfig:
        raise ValueError(
            f"Transformers model type {WENHAO_PSI_PPO_MODEL_TYPE!r} is already registered"
        )
    registered = ModelRegistry._MODEL_REGISTRY.get(WENHAO_PSI_PPO_MODEL_TYPE)
    if registered is None:
        ModelRegistry.register_model(WenhaoPsiPPOModel, IdentityWeightMapper)
    elif registered is not WenhaoPsiPPOModel:
        raise ValueError(
            f"Cosmos model type {WENHAO_PSI_PPO_MODEL_TYPE!r} is already registered"
        )
...
def load_wenhao_rollout_model(
    ...
    Args:
        model_root: Exact pinned Wenhao model directory under ``models/``.
    ...
    if dtype is not torch.bfloat16:
        raise ValueError("Wenhao rollout v1 requires dtype=torch.bfloat16")
    if device.type != "cuda":
        raise ValueError("Wenhao rollout v1 requires one CUDA device")
    if torch.distributed.is_initialized() and torch.distributed.get_world_size() != 1:
        raise NotImplementedError("Wenhao rollout v1 is single-GPU only")
    ...
    with torch.device("meta"), cosmos_default_dtype(dtype):
        model = WenhaoPsiPPOModel(config)
    model._apply(
        lambda tensor: (
...
def _is_attested_model_root(model_name_or_path: str) -> bool:
    """Match only paths shaped like the single pinned Wenhao model root."""
    path = Path(model_name_or_path).expanduser()
    return path.name == MODEL_ID and path.parent.name == "models"


def _config_from_attested_model_root(model_name_or_path: str) -> WenhaoPsiPPOConfig:
    """Create the v1 config after fully attesting a local Wenhao model root."""
    model_root = Path(model_name_or_path).expanduser().resolve(strict=True)
    policy_eval_root = model_root.parent.parent
    bundle = WenhaoSourceBundle.verify(policy_eval_root)
    if bundle.model_root != model_root:
        raise ValueError(
            "Wenhao local config path differs from the verified model root"
        )
...
def _read_attested_run_config(bundle: WenhaoSourceBundle) -> Mapping[str, Any]:
    ...
    if not isinstance(raw, Mapping):
        raise TypeError("Wenhao run_config.json root must be a mapping")
    return raw


def _validate_training_contract(
    run_config: Mapping[str, Any], config: WenhaoPsiPPOConfig
) -> None:
    ...
    if not isinstance(model, Mapping) or not isinstance(field, Mapping):
        raise TypeError("Wenhao run config model/field entries must be mappings")
    expected = {
        "action_dim": 38,
    ...
    for key, value in expected.items():
        if model[key] != value:
            raise ValueError(
                f"Wenhao run config {key!r} must be {value!r}, got {model[key]!r}"
            )
    if model.get("rtc") is not True:
        raise ValueError("Wenhao v1 requires an RTC-trained checkpoint")
    if _rtc_max_delay_exclusive(run_config) != config.rtc_max_delay_exclusive:
        raise ValueError(
            "Wenhao run config max_delay does not match the attested PPO config"
        )
    if field["action_norm_type"] != config.normalization_type:
        raise ValueError("Wenhao run config normalization does not match PPO config")
    if field["use_norm_mask"] is not False:
        raise ValueError("Wenhao v1 requires use_norm_mask=false")
    if run_config["train"]["lora"] is not False:
        raise NotImplementedError("Wenhao Psi v1 does not support LoRA checkpoints")
...
def _rtc_max_delay_exclusive(run_config: Mapping[str, Any]) -> int:
    ...
    if not isinstance(model, Mapping):
        raise TypeError("Wenhao run config model entry must be a mapping")
    if model.get("rtc") is not True:
        raise ValueError("Wenhao v1 requires an RTC-trained checkpoint")
    ...
    if (
        ...
        raise ValueError(
            "Wenhao attested run config must declare exclusive RTC max_delay="
            f"{_NATIVE_RTC_MAX_DELAY_EXCLUSIVE}"
        )
    return value
...
def _construct_psi_architecture(
    *,
    source_bundle: WenhaoSourceBundle,
    ...
    if vlm_config.model_type != "qwen3_vl":
        raise ValueError("Pinned Wenhao v1 bundle requires a qwen3_vl backbone")
    if int(vlm_config.text_config.hidden_size) != vlm_hidden_dim:
        raise ValueError("Pinned Wenhao VLM hidden width does not match PPO config")
...
def _import_attested_psi_locked(
    ...
    *,
    ...
    if snapshot.sha256 != expected_tree_sha256:
        raise ValueError(
            "Wenhao Psi source tree SHA256 mismatch at import: expected "
...
def _require_single_gpu(parallel_dims: Any) -> None:
    ...
    if parallel_dims is None:
        raise ValueError("Wenhao Psi v1 requires explicit Cosmos parallel dimensions")
    values = {
    ...
    if any(value != 1 for value in values.values()):
        raise NotImplementedError(
            f"Wenhao Psi v1 is single-GPU only, got parallel dimensions {values}"
        )
