...
"""Rollout handle for Wenhao HumanoidPolicyService callbacks."""
...
def unwrap_actor_critic(model: torch.nn.Module) -> WenhaoPsiActorCritic:
    """Return the exact Wenhao actor-critic from one supported model owner.

    Cosmos owns a ``WenhaoPsiPPOModel`` wrapper while lightweight rollout tests
    ...
    versioning.
    """
    if isinstance(model, WenhaoPsiActorCritic):
        return model
    from alpagym_g1_wenhao_vla.cosmos_model import WenhaoPsiPPOModel

    if isinstance(model, WenhaoPsiPPOModel):
        return model.actor_critic
    raise TypeError(
        "expected WenhaoPsiActorCritic or WenhaoPsiPPOModel, got "
        f"{type(model).__name__}"
    )


class WenhaoNativeInferenceModel:
    ...
    def sample_trajectories_from_data(
        ...
        raise NotImplementedError(
            "Wenhao rollouts call the leased model from HumanoidPolicyService; "
    ...
    def build_policy_replay_data(
        ...
        raise NotImplementedError(
            "Wenhao replay is emitted atomically with its model action chunk"
        )
