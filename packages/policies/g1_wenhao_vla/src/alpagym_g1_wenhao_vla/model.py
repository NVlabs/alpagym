...
"""Trainable actor-critic core around Wenhao's Qwen3-VL/Psi0 model."""
...
class WenhaoPolicySample:
    ...
    def __post_init__(self) -> None:
        ...
        if not torch.equal(self.trace.chain[:, -1], self.density_latent):
            raise ValueError("Wenhao sample density latent differs from trace final")
        if (
            tuple(self.values.shape) != (batch,)
            or not torch.isfinite(self.values).all()
        ):
            raise ValueError("Wenhao sample values must be finite [B]")
...
class WenhaoPsiActorCritic(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            psi_model: Loaded Wenhao ``Psi0Model`` with its VLM parameters
                already frozen. Its condition path and trainable action head
                are called directly.
            schedule: Exact Wenhao model-timestep and sigma grid used during
                rollout.
        ...
        """
        super().__init__()
        if float(noise_level) != WENHAO_FLOW_NOISE_LEVEL:
            raise ValueError(
        ...
        if (
            ...
            raise ValueError(
                "Wenhao RTC max_delay must be an integer in [2, 30] with an "
    ...
    def train(self, mode: bool = True) -> WenhaoPsiActorCritic:
    ...
    def materialize_action_views(
        ...
        if latent_action.ndim < 2 or tuple(latent_action.shape[-2:]) != (30, 38):
            raise ValueError("Wenhao latent action must end in [30, 38]")
        if (
            not torch.is_floating_point(latent_action)
            or not torch.isfinite(latent_action).all()
        ):
            raise ValueError("Wenhao latent action must be finite floating point")
    ...
    def sample_actions(
        ...
        *,
        ...
        if physical_states.ndim < 1 or int(physical_states.shape[0]) != batch:
            raise ValueError("Wenhao physical_states batch differs from Qwen inputs")
        ...
        *,
        ...
        if teacher_model is not None:
            raise NotImplementedError(
                "Wenhao reference-model KL is not implemented for Flow-SDE replay"
            )
        if flow_schedule_sha256 != self.schedule.sha256:
            raise ValueError("Wenhao replay flow schedule identity changed")
        self.psi_model.eval()
        self._validate_rtc_inputs(
            rtc_prefix_normalized_actions, rtc_prefix_mask, batch=input_ids.shape[0]
        )
        ...
        if tuple(element_log_probs.shape) != expected_element_shape:
            raise ValueError(
                "Wenhao Flow-SDE element log-probabilities must be [B,30,38]"
            )
    ...
    def _validate_rtc_inputs(
        ...
        *,
        ...
        if tuple(rtc_prefix_normalized_actions.shape) != (batch, 30, 38):
            raise ValueError("Wenhao RTC action buffer must have shape [B,30,38]")
        if tuple(rtc_prefix_mask.shape) != (batch, 30):
            raise ValueError("Wenhao RTC mask must have shape [B,30]")
        if rtc_prefix_mask.dtype is not torch.bool:
            raise TypeError("Wenhao RTC mask must use bool dtype")
        if not torch.isfinite(rtc_prefix_normalized_actions).all():
            raise ValueError("Wenhao RTC action buffer contains non-finite values")
        prefix_lengths = rtc_prefix_mask.sum(dim=1)
        if bool((prefix_lengths >= self.rtc_max_delay_exclusive).any()):
            raise ValueError(
                "Wenhao RTC prefix length exceeds the attested checkpoint "
    ...
    def clone_for_inference_lease(self) -> WenhaoPsiActorCritic:
        ...
        if unexpected_trainable:
            raise RuntimeError(
                "Wenhao inference leases can share only a frozen VLM; trainable "
    ...
    def _validate_action_views(
        ...
        *,
        ...
        if latent_chain.ndim != 4 or tuple(latent_chain.shape[-2:]) != (30, 38):
            raise ValueError("Wenhao latent_chain must have shape [B, N+1, 30, 38]")
        batch = int(latent_chain.shape[0])
        expected_shape = (batch, 30, 38)
        if tuple(clipped_normalized_actions.shape) != expected_shape:
            raise ValueError(
                "Wenhao clipped_normalized_actions must have shape [B, 30, 38]"
            )
        if tuple(denormalized_wire_actions.shape) != expected_shape:
            raise ValueError(
                "Wenhao denormalized_wire_actions must have shape [B, 30, 38]"
            )
        ...
        if not torch.isfinite(clipped).all() or not torch.isfinite(wire).all():
            raise ValueError("Wenhao executable action views must be finite")
        if not torch.allclose(clipped, expected_clipped, rtol=1.0e-5, atol=1.0e-6):
            raise ValueError(
                "Wenhao clipped normalized action does not match final latent"
            )
        if not torch.allclose(wire, expected_wire, rtol=1.0e-5, atol=1.0e-6):
            raise ValueError(
                "Wenhao denormalized wire action does not match q01/q99 statistics"
            )
    ...
    def _validate_packed_condition(
        ...
        *,
        ...
        if input_ids.ndim != 2 or attention_mask.shape != input_ids.shape:
            raise ValueError("Wenhao token inputs must have matching [B, S] shapes")
        batch, padded_length = input_ids.shape
        if pixel_values.ndim != 2:
            raise ValueError("Wenhao packed pixel_values must have shape [P, F]")
        if image_grid_thw.ndim != 2 or image_grid_thw.shape[1] != 3:
            raise ValueError("Wenhao packed image_grid_thw must have shape [I, 3]")
        integer_tensors = {
            "sequence_lengths": sequence_lengths,
            "image_counts": image_counts,
            "image_offsets": image_offsets,
        ...
        for name, tensor in integer_tensors.items():
            if tensor.dtype is torch.bool or torch.is_floating_point(tensor):
                raise TypeError(f"Wenhao packed {name} must use an integer dtype")
        if tuple(sequence_lengths.shape) != (batch,) or not torch.all(
            (sequence_lengths > 0) & (sequence_lengths <= padded_length)
        ):
            raise ValueError("Wenhao sequence_lengths must be valid [B] lengths")
        expected_attention = (
        ...
        if not torch.equal(
            ...
            raise ValueError("Wenhao image_patch_counts do not match image_grid_thw")
        ...
        if not torch.equal(expected_patch_counts, patch_counts):
            raise ValueError("Wenhao per-sample image and patch boundaries disagree")
