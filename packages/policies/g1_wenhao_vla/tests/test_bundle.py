"""Strict replay parsing and policy-owned collation for Wenhao Psi0."""
...
from alpagym_g1_wenhao_vla.bundle import (
    MODEL_FAMILY,
    REPLAY_SCHEMA,
    build_data_packer,
    build_model_inputs,
...
def _replay(
    *,
    ...
    payload = {
        ...
        "humanoid": {"wenhao_raw_action_rows": wire_actions.clone()},
    }
...
def test_parser_normalizes_uncompressed_visual_history_to_identity_pooling() -> None:
    ...
    torch.testing.assert_close(
        ...
        replay.payload["humanoid"]["wenhao_raw_action_rows"],
        rtol=0,
        atol=0,
    )
...
def test_bundle_wires_ragged_collator_for_different_history_lengths(
    ...
    inspect.signature(WenhaoPsiActorCritic.forward).bind(object(), **forward_inputs)
...
def test_parser_rejects_schedule_scalar_logprob_and_wire_drift() -> None:
    ...
    raw_rows = humanoid["wenhao_raw_action_rows"].clone()
    raw_rows[0, 0] += 1.0
    humanoid["wenhao_raw_action_rows"] = raw_rows
...
def test_schedule_identity_is_required_and_loader_is_policy_owned(
    ...
    monkeypatch.setattr(
        "alpagym_g1_wenhao_vla.bundle.load_wenhao_rollout_model",
        lambda *args, **kwargs: sentinel,
    )
    monkeypatch.setattr(
        "alpagym_g1_wenhao_vla.bundle.WenhaoNativeInferenceModel",
