...
"""Tests for the Cosmos lifecycle around the attested Wenhao Psi model."""
...
import alpagym_g1_wenhao_vla.cosmos_model as cosmos_model
...
def _config(root: Path) -> cosmos_model.WenhaoPsiPPOConfig:
    """Build a fully pinned test config."""
    schedule = _schedule()
    return cosmos_model.WenhaoPsiPPOConfig(
...
def tiny_source(
...
) -> tuple[Path, WenhaoSourceBundle, dict[str, torch.Tensor]]:
    ...
    monkeypatch.setattr(
        cosmos_model.WenhaoSourceBundle,
        "verify",
    ...
    cosmos_model.register_wenhao_psi_ppo_model()
    return root, bundle, state


def _build_materialized(
    root: Path,
) -> cosmos_model.WenhaoPsiPPOModel:
    """Emulate Cosmos meta construction and CPU materialization."""
    config = _config(root)
    with torch.device("meta"):
        model = cosmos_model.WenhaoPsiPPOModel(config)
...
def test_meta_lifecycle_strict_load_and_trainable_ownership(
    tiny_source: tuple[Path, WenhaoSourceBundle, dict[str, torch.Tensor]],
) -> None:
...
def test_config_and_cosmos_registration(
    ...
    root, bundle, _expected = tiny_source
    assert (
        ModelRegistry._MODEL_REGISTRY[cosmos_model.WENHAO_PSI_PPO_MODEL_TYPE]
        is cosmos_model.WenhaoPsiPPOModel
    )
    config_dir = tmp_path / "hf_config"
    _config(root).save_pretrained(config_dir)
    loaded = AutoConfig.from_pretrained(config_dir)
    assert isinstance(loaded, cosmos_model.WenhaoPsiPPOConfig)

    local = load_model_config(str(bundle.model_root))
    assert isinstance(local, cosmos_model.WenhaoPsiPPOConfig)
...
def test_rollout_loader_rejects_unattested_runtime_shape(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="bfloat16"):
        cosmos_model.load_wenhao_rollout_model(
