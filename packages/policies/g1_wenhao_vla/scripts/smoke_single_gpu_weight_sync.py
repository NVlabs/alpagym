...
"""Run an opt-in real-checkpoint Wenhao PPO and weight-sync smoke on one GPU."""
...
from alpagym_g1_wenhao_vla.cosmos_model import load_wenhao_rollout_model
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Load two attested Wenhao models, take one real PPO optimizer step, "
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Pinned Wenhao model directory under alpa_policy_eval/models.",
    )
...
def main() -> None:
    ...
    if device.type != "cuda" or not torch.cuda.is_available():
        raise ValueError("the Wenhao weight-sync smoke requires one CUDA GPU")
    ...
    trainer = load_wenhao_rollout_model(model_root, device, torch.bfloat16)
    rollout = load_wenhao_rollout_model(model_root, device, torch.bfloat16)
    ...
    if not trainable_names:
        raise RuntimeError("Wenhao model exposes no trainable parameters")
    with torch.no_grad():
    ...
    engine = InferenceEngine(
        WenhaoNativeInferenceModel(rollout),
    ...
    core0 = lease0.model
    if not isinstance(core0, WenhaoPsiActorCritic):
        raise TypeError("Wenhao lease did not return its actor-critic core")
    ...
    if any(
        ...
        raise RuntimeError("Wenhao replay omitted PPO outputs")
    ...
    if action_parameter_ids & critic_parameter_ids:
        raise RuntimeError("Wenhao optimizer parameter groups overlap")
    if action_parameter_ids | critic_parameter_ids != trainable_parameter_ids:
        raise RuntimeError("Wenhao optimizer groups do not cover trainable parameters")
    ...
    if trainer_elements_after is None or trainer_values_after is None:
        raise RuntimeError("post-update Wenhao replay omitted PPO outputs")
    ...
    core1 = lease1.model
    if not isinstance(core1, WenhaoPsiActorCritic):
        raise TypeError("Wenhao lease did not return its actor-critic core")
