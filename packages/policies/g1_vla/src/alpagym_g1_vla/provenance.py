...
class VlaSourceBundle:
    ...
    @classmethod
    def verify(cls, policy_eval_root: str | Path) -> "VlaSourceBundle":
        """Resolve and attest the current pinned workstation bundle."""
        ...
        _require_digest(checkpoint, MODEL_SHA256, label="VLA model checkpoint")
        _require_digest(
            model_root / "run_config.json",
            RUN_CONFIG_SHA256,
            label="VLA run config",
        )
        _require_digest(model_root / "argv.txt", ARGV_SHA256, label="VLA argv")
        _require_digest(stats, STATS_SHA256, label="VLA normalization stats")
