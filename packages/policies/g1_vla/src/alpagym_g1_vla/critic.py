...
"""Observation-only critic overlay for the VLA Psi0 actor."""
...
class VlaValueModel(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if min(vlm_hidden_dim, state_dim, action_dim, prefix_embed_dim) <= 0:
            raise ValueError("VLA critic dimensions must be positive")
    ...
    def forward(
        ...
        if vlm_hidden.ndim != 3:
            raise ValueError("VLA critic VLM hidden must be [B, S, D]")
        ...
        if tuple(attention_mask.shape) != (batch, sequence_length):
            raise ValueError("VLA critic attention mask must be [B, S]")
        ...
        if tuple(state.shape) != (batch, self.state_dim):
            raise ValueError("VLA critic state must be [B, 29] or [B, 1, 29]")
        ...
        if (
            ...
            raise ValueError("VLA critic RTC prefix tensors have incompatible shapes")
        if not all(
            torch.isfinite(tensor).all()
            for tensor in (vlm_hidden, state, prefix_actions)
        ):
            raise ValueError("VLA critic input contains non-finite values")
        ...
        if tuple(value.shape) != (batch,) or not torch.isfinite(value).all():
            raise FloatingPointError("VLA critic returned an invalid value")
        return value
