...
"""Replayable Flow-SDE sampling for VLA's non-uniform Psi0 schedule.

The transition distribution follows RLinf's pinned piRL OpenPI sampler, while
the sigma/model-timestep grid remains the exact grid materialized by VLA's
...
VLA_FLOW_NOISE_LEVEL = 0.4
VLA_FLOW_IGNORE_LAST = True


@dataclass(frozen=True)
class VlaFlowSchedule:
    ...
    def __post_init__(self) -> None:
        ...
        if model_timesteps.ndim != 1 or sigmas.ndim != 1:
            raise ValueError("VLA flow schedule tensors must be one-dimensional")
        if sigmas.numel() != model_timesteps.numel() + 1:
            raise ValueError("VLA flow schedule requires N timesteps and N+1 sigmas")
        if model_timesteps.numel() < 2:
            raise ValueError("VLA flow schedule requires at least two denoise steps")
        if (
            not torch.isfinite(model_timesteps).all()
            or not torch.isfinite(sigmas).all()
        ):
            raise ValueError("VLA flow schedule contains non-finite values")
        if not torch.all(sigmas[:-1] > sigmas[1:]):
            raise ValueError("VLA flow sigmas must be strictly decreasing")
        if not torch.isclose(sigmas[0], torch.tensor(1.0), atol=1.0e-6):
            raise ValueError("VLA flow schedule must start at sigma=1")
        if not torch.isclose(sigmas[-1], torch.tensor(0.0), atol=1.0e-7):
            raise ValueError("VLA flow schedule must end at sigma=0")
    ...
    def to(self, device: torch.device | str) -> VlaFlowSchedule:
...
class VlaFlowTrace:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.schedule_sha256) != 64:
            raise ValueError("VLA flow trace schedule digest must be SHA-256")
...
def _schedule_mean_std(
    ...
    *,
    ...
    if x_t.shape != velocity.shape:
        raise ValueError("VLA flow latent and velocity shapes differ")
    if noise_level <= 0.0:
        raise ValueError("VLA Flow-SDE noise_level must be positive")
    ...
    if not torch.all(delta > 0):
        raise ValueError("VLA Flow-SDE transition requires decreasing sigmas")
...
def _validated_prefix(
    ...
    if prefix_actions is None or prefix_mask is None:
        raise ValueError("VLA RTC prefix actions and mask must be provided together")
    ...
    if tuple(actions.shape) != (batch, horizon, action_dim):
        raise ValueError("VLA RTC prefix actions must match the flow latent")
    if tuple(mask.shape) != (batch, horizon):
        raise ValueError("VLA RTC prefix mask must have shape [B, H]")
    if not torch.isfinite(actions).all():
        raise ValueError("VLA RTC prefix actions contain non-finite values")
    return actions, mask
...
def sample_flow_sde(
    ...
    *,
...
) -> tuple[torch.Tensor, VlaFlowTrace]:
    ...
    if initial_noise.ndim != 3 or not torch.isfinite(initial_noise).all():
        raise ValueError("VLA initial_noise must be finite [B, H, A]")
    ...
    if tuple(denoise_indices.shape) != (batch,) or not torch.all(
        ...
        raise ValueError(
            "VLA Flow-PPO denoise_indices must be [B] and exclude the final "
            "low-variance transition"
        )
    if batch > 1 and not bool((denoise_indices == denoise_indices[0]).all()):
        raise ValueError(
            "VLA RLinf sampling requires one shared denoise index per call"
        )
...
def sample_flow_ode(
    ...
    *,
    ...
    """Run VLA's deterministic Euler path on the exact diffusers grid."""
    if initial_noise.ndim != 3 or not torch.isfinite(initial_noise).all():
        raise ValueError("VLA initial_noise must be finite [B, H, A]")
    batch = int(initial_noise.shape[0])
    schedule = schedule.to(initial_noise.device)
...
def replay_flow_logprob(
    ...
    *,
    ...
    if trace.schedule_sha256 != schedule.sha256:
        raise ValueError("VLA replay schedule identity changed")
...
def replay_flow_transition_logprob(
    ...
    *,
    ...
    if latent_chain.ndim != 4 or not torch.isfinite(latent_chain).all():
        raise ValueError("VLA replay latent_chain must be finite [B, N+1, H, A]")
    chain = latent_chain
    batch, chain_steps, _horizon, _action_dim = chain.shape
    schedule = schedule.to(chain.device)
    if chain_steps != schedule.num_steps + 1:
        raise ValueError("VLA replay chain length does not match schedule")
    denoise_indices = torch.as_tensor(
    ...
    if tuple(denoise_indices.shape) != (batch,) or not torch.all(
        ...
        raise ValueError(
            "VLA Flow-PPO replay excludes the final low-variance denoise transition"
        )
    prefix_actions, prefix_mask = _validated_prefix(
    ...
    if prefix_actions is not None and bool(prefix_mask.any()):
        ...
        if not torch.equal(chain[expanded_mask], expected_prefix[expanded_mask]):
            raise ValueError(
                "VLA replay chain does not preserve its deterministic RTC prefix"
            )
